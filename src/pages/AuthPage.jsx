  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'signin') {
        ...
        const result = await signUp(email, password, username.trim(), zone)
        if (result?.confirmationRequired) {
          setMessage('Account created! Check your email to confirm your sign-in. You may need to sign in after confirming.')
          setEmail('')
          setPassword('')
          setUsername('')
        }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 transition-colors">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white dark:bg-slate-900 py-8 px-4 shadow sm:rounded-xl sm:px-10 border border-slate-200 dark:border-slate-800">
          {message && (
            <div className="mb-6 bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-300 p-4 rounded-md text-sm border border-teal-200 dark:border-teal-800/30 flex items-start gap-3">
          ...
          {mode === 'reset' ? (
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="text-center">
                <button type="button" onClick={() => { setMode('signin'); setError(''); setMessage('') }} className="text-sm font-medium text-primary-600 hover:text-primary-500">
                  Back to sign in
                </button>
              </div>
            </form>
              ...
              <div>
                ...
                {mode === 'signin' && (
                  <div className="mt-1 text-right">
                    <button type="button" onClick={() => { setMode('reset'); setError(''); setMessage('') }} className="text-xs text-slate-500 hover:text-primary-600 dark:hover:text-primary-400">
                      Forgot password?
                    </button>
                  </div>
                )}
          ...
          {mode !== 'reset' && (
            <div className="mt-6 text-center">
              <button onClick={() => { setMode(mode === 'signin' ? 'signup' : 'signin'); setError(''); setMessage('') }} className="text-sm font-medium text-primary-600 hover:text-primary-500">
