...
function LeaderRow({ user: u, rank, isCurrentUser }) {
  ...
  return (
    <div className={`flex items-center gap-3 p-3 rounded-xl transition-all ${isCurrentUser ? 'bg-teal-50 dark:bg-teal-500/10 border border-teal-200 dark:border-teal-500/20' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
      <div className={`w-8 text-center text-sm font-bold ${rank <= 3 ? 'text-xl' : 'text-slate-400'}`}>{rankIcon}</div>
      <div className="w-9 h-9 rounded-full bg-gradient-to-br from-teal-600 to-slate-600 flex items-center justify-center text-xs font-bold text-white flex-shrink-0">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-semibold text-slate-900 dark:text-white flex items-center gap-1.5">
          ...
          {isCurrentUser && <span className="text-[9px] bg-teal-100 dark:bg-teal-500/20 text-teal-700 dark:text-teal-400 px-1.5 py-0.5 rounded-full">You</span>}
        </div>
      ...
      <div className="text-right">
        <div className="text-sm font-bold text-amber-500">{u.total_points?.toLocaleString() || 0}</div>
    ...
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">Top civic contributors in Bhopal</p>
        </div>
        {currentUserRank > 0 && (
          <div className="ml-auto text-right">
            <div className="text-xs text-slate-500 dark:text-slate-400">Your rank</div>
            <div className="text-2xl font-bold text-amber-500">#{currentUserRank}</div>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-2">
        {['global', 'zone'].map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-xs font-medium transition-all ${tab === t ? 'bg-teal-600 text-white' : 'text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800/60 hover:text-slate-900 dark:hover:text-slate-200'}`}>
